...
/lib/sysctl.d/*.conf
/etc/sysctl.conf'
  desc 'check', 'Verify OL 8 disables storing core dumps with the following commands:
...
If the returned line does not have a value of "|/bin/false", or a line is not returned and the need for core dumps is not documented with the information system security officer (ISSO) as an operational requirement, this is a finding.'
  desc 'fix', 'Configure OL 8 to disable storing core dumps.
...
$ sudo sysctl --system'
  ...
  tag gid: 'V-248629'
  tag rid: 'SV-248629r1156626_rule'
  tag stig_id: 'OL08-00-010671'
  tag fix_id: 'F-52017r1156625_fix'
  tag cci: ['CCI-000366']
  tag legacy: []
  tag nist: ['CM-6 b']
  tag 'host'
